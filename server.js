...
// express server setup and handling
const express = require('express')
const app = express()
const PORT = 8876
app.use(express.json()); // json stuff
app.use(express.static('public')); // hi ricky this is just to serve the stuff from public folder
...
app.post('/savedata', (req, res) => {
    ...
    fs.readFile(dataPath, 'utf8', (err, jsonString) => {
        ...
        if (dataType === "user") {
            ...
            if (user=="" || user == null){return res.status(404).json({error:'Invalid User'})}
            ...
            if (prblm=="" || prblm == null){return res.status(404).json({error:'Invalid Problem ID'})}
...
// Just sending the data to client
